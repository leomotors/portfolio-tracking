...
describe("dayPnlDelta", () => {
  it("is the change in unrealized P/L (value − cost)", () => {
    expect(dayPnlDelta({ cost: 100, value: 120 }, { cost: 100, value: 110 })).toBe(
      10,
    );
  });

  it("ignores deposits that raise cost and value equally", () => {
    expect(dayPnlDelta({ cost: 200, value: 220 }, { cost: 100, value: 120 })).toBe(
      0,
    );
  });
});
...
describe("findTopAndWorstPerformers", () => {
  const current: AccountSnapshot[] = [
    { id: 1, name: "Alpha", cost: 100, value: 150 },
    { id: 2, name: "Beta", cost: 200, value: 180 },
    { id: 3, name: "Gamma", cost: 50, value: 55 },
  ];
  ...
  it("returns null when no accounts overlap with previous", () => {
    expect(
      findTopAndWorstPerformers(current, new Map([[99, { cost: 1, value: 1 }]])),
    ).toBeNull();
  });
  ...
  it("picks highest and lowest day P/L contribution", () => {
    ...
    expect(findTopAndWorstPerformers(current, previousById)).toEqual({
      top: { name: "Alpha", pnlDelta: 30 },
      worst: { name: "Beta", pnlDelta: -30 },
    });
  });

  it("skips accounts missing from the previous snapshot", () => {
    const prev = new Map([
      [2, { cost: 200, value: 210 }],
      [3, { cost: 50, value: 60 }],
    ]);
    expect(findTopAndWorstPerformers(current, prev)).toEqual({
      top: { name: "Gamma", pnlDelta: -5 },
      worst: { name: "Beta", pnlDelta: -30 },
    });
  });

  it("allows the same account to be both top and worst", () => {
    expect(
      findTopAndWorstPerformers(

...
export const metadata = {
  title: "Sign in — Portfolio Tracking",
};
...
export default async function LoginPage({
  ...
  const startUrl = `/api/auth/login?returnTo=${encodeURIComponent(safeReturnTo)}`;
  ...
  return (
    <main className="flex min-h-screen w-full items-center justify-center bg-[var(--bg)] px-6">
      <div className="w-full max-w-[380px] rounded-[var(--radius-lg)] border border-[var(--hairline)] bg-[var(--surface)] px-7 py-8 shadow-sm">
        ...
        <p className="mt-5 text-[11px] leading-relaxed text-[var(--ink-3)]">
          Access is restricted to authorized Discord accounts only.
        </p>
      </div>
    </main>
  );
